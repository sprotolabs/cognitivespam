...
async function getMdxContent(slug: string[] | undefined) {
  ...
  const filePath = !slug || slug.length === 0
    ? path.join(process.cwd(), 'content/api-reference/index.mdx')
    : path.join(process.cwd(), 'content/api-reference', `${slug.join('/')}.mdx`);
...
export default async function ApiReferencePage({ params }: { params: { slug?: string[] } }) {
